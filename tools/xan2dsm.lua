...
pipelines, no textures, so there are no `global' entries either.
	...
	lua tools/xan2dsm.lua [-name SYM] [-clips A,B,C] [-clip A] anims.xan > anims.dsm
...
if not path then
	io.stderr:write("usage: xan2dsm.lua [-name SYM] [-clips A,B,C] [-clip A] anims.xan > anims.dsm\n")
	os.exit(1)
end
if not symName then
...
local function main()
	...
	emitf("; %s as an xtc chunk, generated by tools/xan2dsm.lua from %s", symName, path)
